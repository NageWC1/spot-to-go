...
\begin{center}
    ...
    {\large Final Project Report} \\[0.4em]
    {\normalsize (Draft submitted for supervisor review and feedback)} \\[3em]

    \begin{tabular}{rl}
        \textbf{Project type:} & Undergraduate final-year project \\[0.4em]
        \textbf{Platform:}     & Android (Kotlin, Jetpack Compose) \\[0.4em]
        \textbf{Date:}         & August 2026 \\
    \end{tabular}

    \vfill

    {\normalsize A report submitted in partial fulfilment of the requirements} \\
    {\normalsize for the degree programme, describing the design, implementation,} \\
    {\normalsize and current progress of the Spot To Go application.} \\[2em]
\end{center}
\clearpage
...
\caption{UML use case diagram for Spot To Go. The User is the single primary actor; Firebase, the Google Maps platform, and YouTube/TikTok are secondary actors the system relies on. Viewing details \emph{includes} viewing the map, and watching a video or getting directions \emph{extend} the detail use case as optional follow-on actions.}
\label{fig:usecase}
\end{figure}
